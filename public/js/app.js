const app = Vue.createApp({
  data() {
    return { images: [], isTableView: false };
  },
  computed: {
    tableIcon() {
      return `
  ...
  mounted() {
    ...
    // Load second CSS stylesheet
    const bootstrapLink = document.createElement("link");
    bootstrapLink.rel = "stylesheet";
    bootstrapLink.href = "css/bootstrap.css"; // Corrected path to Bootstrap CSS
    document.head.appendChild(bootstrapLink);

    // Load Bootstrap JS bundle
    fetch("js/bootstrap.bundle.js") // Corrected path to Bootstrap JS
      .then((response) => {
        if (response.ok) {
          return response.blob(); // Use blob instead of text for script execution
        } else {
